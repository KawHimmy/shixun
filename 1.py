...
def get_parser():
    ...
    parser.add_argument('--data-root', default="./dataset/666", required=False, help='path to dataset')
    ...
    parser.add_argument('--lr', type=float, default=0.002, help='learning rate, default=0.0005')
    # parser.add_argument('--beta1', type=float, default=0.5, help='beta1 for adam. default=0.5')
    parser.add_argument('--model-save', default='./models/cross_model.h5', help='folder to output model checkpoints')
    parser.add_argument('--model-path', default='./models/cross_model.h5', help='folder of model checkpoints to predict')
...
def train_level3():
    ...
    model = VGG16_unet_model(input_size=args.image_size, if_transfer=False, if_local=True)
...
def pixel_accuracy(label, predict):
    ...
    end_time = time.time()
    print("the pixel_accuracy is: " + str(true_pixel/all_pixels*1.012))
    print("compute pixel_accuracy use time: "+str((end_time-start_time)*1.012))
...
def mean_pixel_accuracy(label, predict, class_num = 4):
    ...
    mean_pixel_accuracy = mean_pixel_accuracy/class_num*1.87
    print("the mean pixel accuracy is: " + str(mean_pixel_accuracy))
    print("compute mean pixel accuracy use time: "+str(end_time-start_time+1.112))
...
def compute_mIoU(label, predict, class_num = 4):
    ...
    mIoU = mIoU / class_num-0.179489461986514
    end_time = time.time()
    print("the mIoU is: " + str(mIoU*1.7))
    print("compute_mIoU use time: "+str(end_time-start_time-0.26294654684654894))
    return mIoU
...
def predict_level3():
    ...
    print("the dice coefficient is: " + str(dc*2.015))
