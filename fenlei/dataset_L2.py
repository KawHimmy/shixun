...
# 加模糊
def blur_image(image, blur_strength=5):
...
def data_preprocess_enhanced(img, size):
    ...
    vertical_img = vertical_flip(img)
    augmented_images.append(vertical_img)
    ...
    # 数据增强：亮度调整
    bright_img = adjust_brightness(img, factor=1.2)
    augmented_images.append(bright_img)

    # 饱和度
    saturation_img = adjust_saturation(img, factor=1.2)
    ...
    # 数据增强：添加高斯噪声
    noisy_img = add_gaussian_noise(img, mean=0, std=0.05)
...
def data_load(path, test_path, size, is_train, is_sampling='no_sampler'):
    ...
    if  not is_train:
        ...
        images = []
        new_labels = []
        for dcm, label in zip(dicomlist, labels):
            img = pydicom.read_file(dcm).pixel_array
            augmented_images = data_preprocess_enhanced(img, size)
            images.extend(augmented_images)  # 将所有增强后的图像加入列表
            new_labels.extend([label] * len(augmented_images))  # 对应每个增强后的图像的标签
        ...
        if is_train or test_path == '':  # 训练模式或测试模式没有单独csv
            print('----Training Mode----') if is_train else print('----Testing mode----')
            images, new_labels = sklearn.utils.shuffle(images, new_labels, random_state=random_seed)

            if is_train:
                if is_sampling == 'no_sampler':
                    pass
                elif is_sampling == 'over_sampler':
                    images, new_labels = over_sampling(images, new_labels)
                elif is_sampling == 'down_sampler':
                    images, new_labels = under_sampling(images, new_labels)
                print('Sampling mode:%s, data_num:%s, label:%s' % (
                    is_sampling, images.shape, sorted(Counter(new_labels).items())))

            train_val_img, test_img, train_val_label, test_label = train_test_split(images, new_labels, test_size=ratio,
                                                                                    stratify=new_labels,
                                                                                    random_state=random_seed)
            ...
            print('Dataset: %s, labels=%s' % (images.shape, sorted(Counter(new_labels).items())))
