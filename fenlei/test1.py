...
import math
from sklearn.metrics import precision_recall_curve, classification_report, roc_auc_score, confusion_matrix, f1_score, \
...
# Precision-Recall 曲线
def plot_precision_recall_curve(true_labels, predicted_probs, model_path):
...
def for_test_resnet(best_model_name):
    ...
    plot_precision_recall_curve(test_true, test_prob, model_path)
...
# 随机选择一张测试图像进行展示和预测
def display_random_test_image(test_img, test_true, test_pred):
    # 随机选择索引
    random_index = random.randint(0, len(test_img) - 1)

    # 获取图像和真实标签
    selected_image = test_img[random_index]
    true_label = test_true[random_index]
    predicted_label = test_pred[random_index]

    # 如果 selected_image 是 3D（例如 [channel, height, width]），则转换为 2D
    if selected_image.ndim == 3:
        selected_image = selected_image.squeeze()  # 移除不必要的维度
...
# 在测试完成后调用此函数
display_random_test_image(test_img, test_true, test_pred)
